...
INVALID_OUTPUTS = ['none','space','del']
...
def print_result(result: GestureRecognizerResult, output_image: mp.Image, timestamp_ms: int):
    ...
    global temp
    if category_name.lower() in INVALID_OUTPUTS or category_name == temp: return
    temp = category_name
...
with GestureRecognizer.create_from_options(options) as RECOGNIZER:
    while cap.isOpened():
        ...
        # Show to screen
        cv2.imshow('Gesture-Recognizer: American Sign language (ASL)', cv2.flip(frame, 1))
